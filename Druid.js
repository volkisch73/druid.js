...
var ClassAttack = {
	...
	doCast: function (unit, timedSkill, untimedSkill) {
		...
		if (timedSkill > -1 && (!me.getState(121) || !Skill.isTimed(timedSkill))) {
			switch (timedSkill) {
			case 243: // Poison Nova
				if (!this.shockwaveTick || getTickCount() - this.shockwaveTick > Config.ShockwaveDelay * 1000) {
					...
					if (!unit.dead && Skill.cast(timedSkill, Skill.getHand(timedSkill), unit)) {
						this.shockwaveTick = getTickCount();
					}
				}
		...
		while (this.shockwaveTick && getTickCount() - this.shockwaveTick < Config.ShockwaveDelay * 1000) {
			delay(40);
		}
